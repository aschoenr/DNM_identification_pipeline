...
#return pid + ' ' + ct + ' ' + ut + ' '  + vm + ' ' + rm + ' ' + tmp_du
#ct = time since epoch in seconds
#ut = CPU usage (percentage of total number of CPUs)
#vm = virtual memory usage in kb
#rm = real memory usage in kb
#tmp_du = temp directory disk usage in kb 


temp_dir = settings.tmp_dir #'/home/aschoenr/tmp/'
page_size = 4096 #os.sysconf('SC_PAGESIZE')

def get_all_children(pid):
    parents = os.popen('pgrep -P ' + str(pid)).read()
    parents = parents.strip().split()
    children = []

    while (len(parents) > 0):
        children.append(parents[0])
        temp = os.popen('pgrep -P ' + str(parents[0])).read()
        parents.remove(parents[0])
        parents += temp.strip().split()
...
def process_info(pid):
    ...
    if os.path.isfile(filename): #my_file.is_file():
        f = open('/proc/' + pid + '/stat', 'r')
        data = f.readline()
        f.close()

        data = data.strip().split()
        ut = long(data[13])  + int(data[14])
        vm = long(data[22])
        rm = long(data[23]) * page_size
...
def get_stats(pid):
    ...
    pid = str(pid)
    try:
        data = subprocess.check_output("du -sk " + temp_dir, shell=True)
    except:
        return ''
    tmp_du = data.split()[0]

    #ps_cmd = 'ps -p ' + pid + ' -o cputime,etime | tail -1'


    #data = subprocess.check_output('ps aux | grep ' + pid + ' | grep -v grep', shell=True)
    #data = subprocess.check_output(ps_cmd, shell=True)
    #data = data.split()    
    #print data
    #cpu = data[2]
    #vm  = data[4]
    ...
    tt1 = total_time()
    ut1,vm1,rm1 = process_info(pid)

    time.sleep(1)

    tt2 = total_time()
    ut2,vm2,rm2 = process_info(pid)

    if not((ut1 == -1 and vm1 == -1 and rm1 == -1) or (ut2 == -1 and vm2 == -1 and rm2 == -1)):
        #user_util = 100 * (utime_after - utime_before) / (time_total_after - time_total_before);
        #sys_util = 100 * (stime_after - stime_before) / (time_total_after - time_total_before);

        ut = str("{0:.4f}".format(100 * (ut2 - ut1) / float(tt2 - tt1)))
        vm = str("{0:.0f}".format(((vm1 + vm2) / 2) / 1024.0))
        rm = str("{0:.0f}".format(((rm1 + rm2) / 2) / 1024.0))
    
        ct = str(calendar.timegm(time.gmtime()))
        #print str(ut1) + ' ' + str(ut2) + ' ' + str(tt1) + ' ' + str(tt2)
        #print str(ut2 - ut1) + ' ' + str(tt2 - tt1)
        return pid + ' ' + ct + ' ' + ut + ' '  + vm + ' ' + rm + ' ' + tmp_du
    return ''

def main():
    while True:
        print get_stats('37914')
        time.sleep(9)
...
/proc/[pid]/stat
              ...
              flags %u (%lu before Linux 2.6.22)
                          ...
                          PF_* defines in <linux/sched.h>.  Details depend on the kernel  ver-
                          sion.
             ...
             13 utime %lu   Amount  of  time  that this process has been scheduled in user mode,
              ...
              priority %ld
                          ...
                          For  processes running under a non-real-time scheduling policy, this
                          is the raw nice value (setpriority(2)) as represented in the kernel.
                          The kernel stores nice values as numbers in the range 0 (high) to 39
            ...
            23  rss %ld     Resident Set Size: number of pages the process has in  real  memory.
              ...
              signal %lu  The bitmap of pending signals, displayed as a decimal number.  Obso-
                          lete, because it does not provide information on real-time  signals;
                          use /proc/[pid]/status instead.

              blocked %lu The bitmap of blocked signals, displayed as a decimal number.  Obso-
                          lete, because it does not provide information on real-time  signals;
                          use /proc/[pid]/status instead.

              sigignore %lu
                          The bitmap of ignored signals, displayed as a decimal number.  Obso-
                          lete, because it does not provide information on real-time  signals;
                          use /proc/[pid]/status instead.

              sigcatch %lu
                          The  bitmap of caught signals, displayed as a decimal number.  Obso-
                          lete, because it does not provide information on real-time  signals;
                          use /proc/[pid]/status instead.
