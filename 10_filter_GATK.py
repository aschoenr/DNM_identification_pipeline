...
def load_vcfs(vcf_snp_results, vcf_indel_results, outdir, out_file=None):
    ...
    for p in vcf_snp_results:
        ...
        for line in f:
            if line[0] != '#':
                if 'INDEL' in line:
                #if (len(line.split('\t')[3].strip()) > 1 or
                #    len(line.split('\t')[4].strip()) > 1):
...
def final_filter(p_id, dng_results, vcf_snp_results, vcf_indel_results, mom, dad, outdir, result_counts):
    ...
    #f = open(outdir + p_id + '.dnm', 'w')

    #print 'missing vcf results'
    missing_vcf_data = set([])

    for r in dng_results[p_id]:
        #missing_vcf_data = set([])
 
        indel = (len(line.split('\t')[3].strip()) > 1 or len(line.split('\t')[4].strip()) > 1)
    ...
    if len(missing_vcf_data) > 0: 
        print 'missing vcf results: ' + p_id + ' ' + str(len(missing_vcf_data))
...
indir  = settings.indirroot  + '9_genotyperefinement/' + fam_id + '/'
outdir = settings.indirroot + '10_filtered_GATK/' + fam_id + '/'
...
for k in kids:
    ...
    f = open(indir + k + '.postCGP.Gfiltered.DNMs.vcf', 'r')
    pre_f = open(outdir + k + '_pre-filter.vcf', 'w')
    raw_counts[k] = 0

    for line in f:

        if 'ConfDeNovo' in line:
            raw_counts[k] += 1

        if (line.startswith('#') or
            line.split('\t')[6] != 'PASS' or
            'ConfDeNovo' not in line or
            'lowGQ' in line):
        ...
        result_counts[k][0] += 1
        read_count = int(line.split('\t')[-1].split(':')[2])
        if read_count <= coverage_cutoff[k]:                                # filter high read counts
            if not in_lcr(line.split('\t')[0], int(line.split()[1]), lcrs):   # filter low complexity
                dng_results[k].append(line.strip())
            else:
                result_counts[k][2] += 1
        else:
            result_counts[k][1] += 1
...
print "\nid\traw_dnm_calls\ttotal_GATK_calls\thigh_read_count\tlow_complexity\tbidirectional\tparent_allele"

for k in kids:
    final_filter(k, dng_results, vcf_snp_results, vcf_indel_results, mom, dad, outdir, result_counts)
    out = k + '\t' + str(raw_counts[k])
